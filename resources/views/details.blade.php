      ...
      <div class="row">
        ...
        <div class="col-lg-5">
          ...
          @if (Cart::instance('cart')->content()->where('id',$product->id)->count()>0)
                <a href="{{route('cart.index')}}" class="btn btn-warning mp-3">Go to cart</a>
            @else
          ...
          <div class="product-single__addtolinks">
            ...
            <form method="POST" action="{{route('wishlist.item.remove',['rowId'=>Cart::instance('wishlist')->content()->where('id',$product->id)->first()->rowId])}}">
                        @csrf
                        @method('DELETE')
            <button type="submit" class="btn menu-link menu-link_us-s add-to-wishlist"><svg width="16" height="16" viewBox="0 0 20 20"
                fill="none" xmlns="http://www.w3.org/2000/svg" style="color:orange">
                <use href="#icon_heart" />
              </svg> <span style="color: red">Remove from Wishlist</span></button>
            </form>
            @else
